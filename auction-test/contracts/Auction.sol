...
contract Auction {
    ...
    constructor(
        ...
        auctionInfo = AuctionInfo({
            ...
            hiddenBidsDeadline: block.timestamp + 1 days,
            openBidsDeadline: block.timestamp + 2 days
        });
    ...
    function closeOpenRound() public inState(State.ReadyForOpenBids) isAfterDeadline(auctionInfo.openBidsDeadline) {
    ...
    function transferHighestBidToSeller() internal inState(State.Closed) {
        ...
        if (highestBid > auctionInfo.depositValue) {
            ...
            eventMsg = "The highest bid was higher than the deposit value. Transferring the deposit to seller";
        }
    ...
    function getAuctionInfo() public view returns(State, address, uint, uint, uint, uint, uint) {
        return (
            ...
            auctionInfo.seller,
            auctionInfo.energyAmount,
            auctionInfo.minBidValue,
            auctionInfo.depositValue,
            auctionInfo.hiddenBidsDeadline,
            auctionInfo.openBidsDeadline
        );
    }
